...
begin
  Result := True;
  if Dir = '' then begin
    E := EInOutError.Create('Cannot Create Directory');
    E.ErrorCode := 3;
    raise E;
  end;
...
procedure ConvertErrorFmt(const sError: string; const Args: array of const); local;
begin
  raise EConvertError.CreateFmt(sError, Args);
end;
...
begin
  if (TimeStamp.Time < 0) or (TimeStamp.Date <= 0) or
    ...
    ConvertErrorFmt('''%d.%d'' is not a valid timestamp', [TimeStamp.Date, TimeStamp.Time]);
end;
...
begin
  FMessage := Msg;
//{$ifdef MSWINDOWS}  MsgBox(FMessage); {$endif}
end;
...
begin
  raise EAbort.Create('Operation Aborted') at ReturnAddress;
end;
{$ELSE}
begin
...
function AnsiCompareStr(const S1, S2: string): Integer;
begin
{$ifdef MSWINDOWS}  // !!!: SORT_STRINGSORT need for correctly work binary sort (TStringList.QuickSort)
  Result := CompareString(LOCALE_USER_DEFAULT, SORT_STRINGSORT, pointer(S1), Length(S1),
...
begin
{$ifdef MSWINDOWS} // !!!: SORT_STRINGSORT need for correctly work binary sort (TStringList.QuickSort)
  Result := CompareString(LOCALE_USER_DEFAULT, NORM_IGNORECASE or {!}SORT_STRINGSORT, pointer(S1),
