...
begin
  L := fPointer^;
{$ifdef UNICODE}
  SetString(S,PAnsiChar(fPointer)+1,fPointer^);
  result := string(S); // let the RTL do the conversion
{$else}
...
begin
  ...
  for i := 0 to RegisteredClasses.Count-1 do begin
    ...
    if PShortString(PInteger(integer(result)+vmtClassName)^)^=AClassName then
      exit;
  end;
  result := nil;
end;
...
procedure RegisterClass(AClass: TPersistentClass);
begin
  if FindClass(PShortString(PInteger(integer(AClass)+vmtClassName)^)^)=nil then
